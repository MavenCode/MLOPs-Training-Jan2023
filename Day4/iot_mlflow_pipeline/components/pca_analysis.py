def PCA(clean_data_dir: str, exp_id: str, 
        minio_server, minio_access_key, minio_secret_key) -> str:
    ...
    run_name = f"PCA_component_{datetime.now()}"    
    
    with mlflow.start_run(run_name=run_name, experiment_id=exp_id):
        mlflow.log_artifact(os.path.join(pca_dir,'pca_metrics.pickle'), "pca_analysis")
